...
int main()
{
	...
	{
		pos = 0;
		while ((type = getop_from_line(line,s)) != '\0')
		{
			switch (type)
			{
			...
			{
				value_to_assign = pop();
				int next_token_type = getop_from_line(line, s);
	...
	int getop_from_line(char line[], char  s[])
	{
		int i=0, c, n;
		...
		n = c;
		if (c == '-')
			if (isdigit(c = line[pos++])) /*if next character to - is digit ,then its negative number*/
			{
				s[++i] = c;
